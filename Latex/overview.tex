...
\end{leancode}
  \item Definition \emph{kification} \question{Does this already exist and if no is this good?}\todo{Do the proofs.}
\begin{leancode}
...
\end{leancode}
  \item Lemma \emph{iUnion\_ball\_eq\_levelaux} \todo{Extraxt lemma (marked in code)}
\begin{leancode}
...
\end{leancode}
  ...
  \todo[caption={Unify this lemma and the next.}, size=\footnotesize]
...
def CWComplex_disjointUnion (disjoint : Disjoint C D) : CWComplex (C ∪ D) where
  ...
  union := sorry
\end{leancode}
  \item Definition \emph{CWComplex\_subcomplex} \questionsmall{Is this definition good? Should this be a structure?} \questionsmall{How can I change the Set below to the type of subtypes?}
  \questionsmall{How I do the typecasting correctly in mapsto (see code)?}
\begin{leancode}
...
CWComplex (⋃ (n : ℕ) (j : I n), hC.map n j '' ball 0 1) where
  ...
  union := sorry
\end{leancode}
  \item Definition \question{specific code question (see code)}
\begin{leancode}
...
(hD : @CWComplex Y t2 D)
\end{leancode}
  \item Definition \emph{CWComplex\_product} \todo{Make this work and do the proofs!}\question{How do I tell lean to choose the inferred instance over the synthesized instance?}
  \comment{Floris said to redefine the product first.}
\begin{leancode}
...
\end{leancode}
  \item Lemma \emph{iUnion\_subcomplex} \question{How do I do the typecasts in mapsto?}\todo{Finish the last three proofs.}
\begin{leancode}
