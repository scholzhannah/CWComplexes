...
lemma PartialEquiv.toEquiv_eq_codRestrict_restrict {α β : Type*} (e : PartialEquiv α β) :
    e.toEquiv = codRestrict (e.source.restrict e) e.target (by simp) :=
  rfl

lemma PartialEquiv.toEquiv_eq_codRestrict_restrict_invFun {α β : Type*} (e : PartialEquiv α β) :
    e.toEquiv.symm = codRestrict (e.target.restrict e.invFun) e.source (by simp) := by
  rfl

theorem continuous_codRestrict_iff {X Y : Type*} [TopologicalSpace X] [TopologicalSpace Y]
    {f : X → Y} {s : Set Y} (hs : ∀ a, f a ∈ s) :
...
lemma ContinuousOn.union_of_disjoint_closure {α β : Type*} [TopologicalSpace α]
    [TopologicalSpace β] {f : α → β} {s t : Set α}
...
lemma continuousOn_union_iff_of_disjoint_closure {α β : Type*} [TopologicalSpace α]
    [TopologicalSpace β] {f : α → β} {s t : Set α}
