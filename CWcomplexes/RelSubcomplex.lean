...
-- **Question**: should these be simp lemmas? Since I need to exclude them below
...
instance RelCWComplex.Subcomplex.finite_bot [T2Space X] [RelCWComplex C D] :
  ...
  eventually_isEmpty_cell := by simp [-coe_bot]
  finite_cell n := by simp [Finite.of_subsingleton, -coe_bot]
...
lemma RelCWComplex.Subcomplex.iSup_carrier [T2Space X] [RelCWComplex C D]
  ...
  simp [iSup, CompletelyDistribLattice.coe_MinimalAxioms_sSup]
...
lemma RelCWComplex.Subcomplex.iSup_I [T2Space X] [RelCWComplex C D]
  ...
  simp [iSup, CompletelyDistribLattice.MinimalAxioms_sSup_I]
...
lemma RelCWComplex.Subcomplex.coe_iInf [T2Space X] [RelCWComplex C D]
  ...
  simp [iInf, CompletelyDistribLattice.coe_MinimalAxioms_sInf]
...
lemma RelCWComplex.Subcomplex.iInf_I [T2Space X] [RelCWComplex C D]
    {J : Type*} (sub : J → Subcomplex C) (n : ℕ) :
    (⨅ (j : J), sub j).I n = ⋂ (j : J), (sub j).I n := by
  simp [iInf, CompletelyDistribLattice.MinimalAxioms_sInf_I]
...
lemma RelCWComplex.Subcomplex.finite_attachBase [T2Space X] [RelCWComplex C D] (n : ℕ)
  ...
  eventually_isEmpty_cell := by
      ...
      use n.succ
      intro b le
      simp only [cell_def, attachBase_I, heq_eq_eq, Sigma.mk.injEq, coe_setOf,
  ...
  finite_cell := by
      ...
      by_cases h : m = n
      · subst m
        simp only [cell_def, attachBase_I, heq_eq_eq, Sigma.mk.inj_iff, true_and,
          setOf_eq_eq_singleton]
        exact Finite.of_subsingleton
      · suffices IsEmpty (cell (attachBase n i hD : Set X) m) by
          exact Finite.of_subsingleton
        simp only [cell_def, attachBase_I, heq_eq_eq, Sigma.mk.inj_iff, coe_setOf,
          isEmpty_subtype, not_and]
        intro c eq
        contradiction
...
instance CWComplex.Subcomplex.finite_cellZero [T2Space X] [CWComplex C] (i : cell C 0) :
  ...
  eventually_isEmpty_cell := by
    ...
    use 1
    intro b leb
    simp only [cell_def, cellZero_I, heq_eq_eq,
...
lemma RelCWComplex.Subcomplex.finiteDimensional_attachCell [T2Space X] [RelCWComplex C D] (n : ℕ)
  ...
  eventually_isEmpty_cell := by
    ...
    use a.max (n + 1)
    intro b le
    simp only [cell_def, isEmpty_subtype, attachCell_I, Sigma.mk.inj_iff, coe_setOf,
...
lemma RelCWComplex.Subcomplex.finiteType_attachCell [T2Space X] [RelCWComplex C D] (n : ℕ)
  ...
  finite_cell := by
    ...
    by_cases h : m = n
    · subst m
      simp only [cell_def, attachCell_I, Sigma.mk.inj_iff, heq_eq_eq, true_and,
        setOf_or, setOf_mem_eq, setOf_eq_eq_singleton, finite_coe_iff, finite_union]
      exact ⟨finite.finite_cell n, finite_singleton i⟩
    simp only [cell_def, attachCell_I, Sigma.mk.inj_iff, h, false_and, or_false,
...
lemma RelCWComplex.Subcomplex.cell_mem_finite_subcomplex [T2Space X] [RelCWComplex C D]
  ...
  let := finite_of_finiteDimensional_finiteType (sub' : Set X)
...
lemma RelCWComplex.Subcomplex.finite_iUnion_subset_finite_subcomplex [T2Space X] [RelCWComplex C D]
  ...
  · choose sub finite subset using closedCell_subset_finite_subcomplex (C := C) (D := D)
    let := finite_of_finiteDimensional_finiteType
      ((⨆ (x : Σ n, I n), sub x.fst x.snd : Subcomplex C) : Set X)
...
lemma RelCWComplex.Subcomplex.skeletonLT_eq [T2Space X] [RelCWComplex C D] (E : Subcomplex C)
  ...
  simp only [← iUnion_openCell_eq_skeletonLT, cell_def, openCell_eq, iUnion_coe_set,
    inter_union_distrib_left, inter_eq_right.2 base_subset_complex, inter_iUnion]
  congrm D ∪ ⋃ (m : ℕ) (_ : (m : ℕ∞) < ↑n) (i : cell C m), ?_
  by_cases h : i ∈ E.I m
  · simp only [h, iUnion_true, right_eq_inter]
    rw [← openCell_eq E m ⟨i, h⟩]
    exact openCell_subset_complex _ _
  · simp only [h, iUnion_of_empty]
...
export RelCWComplex.Subcomplex (subset_complex finiteType_subcomplex_of_finiteType
  ...
  instDistribLattice instCompletelyDistribLattice coe_top top_I coe_bot bot_I finite_bot
  iSup_I finiteDimensional_finite_iSup_of_finiteDimensional
  finiteType_finite_iSup_of_finiteType coe_iInf
